 ...
 */
public class UpgradePipelineInitializer implements PipelineInitializer {

    private UpgradeEncodeHandler encodeHandler;

    private UpgradeDecodeHandler decodeHandler;

    private ByteToClientMsgHandler clientMsgHandler;

    public UpgradePipelineInitializer() {
        this.encodeHandler = UpgradeEncodeHandler.INSTANCE;
        this.clientMsgHandler = new ByteToClientMsgHandler(ClientProtocol.FIRMWARE);
        this.decodeHandler = new UpgradeDecodeHandler();
    }
    ...
    public void initPipelineDecoder(ChannelPipeline pipeline, String baseName, TransportType type) {
        if (TransportType.UDP.equals(type)) {
            pipeline.addAfter(baseName, UpgradeDecodeHandler.NAME, decodeHandler);
            return;
        }
    ...
    public String decoderName() {
        return UpgradeDecodeHandler.NAME;
    }
