...
public class UpgradeEncodeHandler extends ChannelOutboundHandlerAdapter {

    public static final UpgradeEncodeHandler INSTANCE = new UpgradeEncodeHandler();

    private UpgradeEncodeHandler() {}
    ...
    public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception {
        ...
        if (msg instanceof ServerMessage) {
            ...
            if (!protocol.equals(ClientProtocol.FIRMWARE)) {
                ctx.writeAndFlush(msg, promise);
                return;
            }
            log.debug("upgrade output client : {}, message ↓↓↓ \r\n{}", message.getPeer(),
                    log.isDebugEnabled() ? ByteBufUtil.hexDump((ByteBuf) message.getMessage()) : null);
            if (TransportType.UDP.equals(message.getTransportType())) {
